...
void change_time(time_t* stime) {   // n can be +1 or -1
    pthread_mutex_lock(&serialize_values5);
    *stime = time(NULL);
    pthread_mutex_unlock(&serialize_values5);
}
pthread_mutex_t serialize_values6 = PTHREAD_MUTEX_INITIALIZER;
void set_value(int* variable, int num) {   // n can be +1 or -1
    pthread_mutex_lock(&serialize_values6);
    *variable = num;
    pthread_mutex_unlock(&serialize_values6);
}
