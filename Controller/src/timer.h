...
extern float incomingRate[];
extern float sum_incoming_rate;
...
//   incoming, outgoing, failing, hostIncomingRate, avg_service_time, proxy2_in, capacity, current_ratio, ctr, flag);*/
    ...
    if(!no_log) {
        fprintf(log_ptr,
                "%s "
                "totalWt %0.2f "    //logging the hostIncomingRate parameter
                "Cap %d "
                "%d "
                "\n",
                time_buf,
                sum_incoming_rate,  //logging the hostIncomingRate parameter
                capacity,
                current_time//,
                );
        fflush(log_ptr);
...
//  }
    ...
    //debug_printf("## Count -- Total numbytes %d\n",tot_numbytes);
    change_values(&incoming, 0);
    change_values(&numbytes, 0);
